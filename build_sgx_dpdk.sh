...
if [ "$MODE" == "debug" ]; then
    ...
    cargo +nightly-2020-05-30 build --release
fi
popd
...
do
	...
	pushd examples/$TASK
	if [ "$MODE" == "debug" ]; then
	    cargo +nightly-2020-05-30 build --target=x86_64-fortanix-unknown-sgx
	else
	    cargo +nightly-2020-05-30 build --target=x86_64-fortanix-unknown-sgx --release
	fi
	popd
